...
/*
    ...
    * VIEW 옵션들
    ...
    - NOFORCE : 서브 쿼리에 기술된 테이블이 존재 해야만 뷰가 생성된다. (기본값)
...
-- 참고 : 접속한 계정이 가지고 있는 VIEW에 대한 정보를 조회하는 뷰테이블
SELECT * FROM USER_VIEWS;

-- '한국'에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가명 조회
SELECT * 
FROM VW_EMPLOYEE
WHERE NATIONAL_CODE = 'KR';
...
SELECT 
    ...
    ROUND((SYSDATE-HIRE_DATE)/365,1)||'년'AS "근무년수"
...
AS SELECT 
    ...
    JOB_NAME, 
    DECODE(SUBSTR(EMP_NO, 8,1), 1, '남자', 2, '여자') AS "성별",
    ROUND((SYSDATE-HIRE_DATE)/365,1)AS "근무년수"
    --EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE)
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE);
...
-- DELETE
DELETE FROM VW_JOB WHERE JOB_NAME = '인턴';
...
-- INSERT (ERROR)
INSERT INTO VW_JOB VALUES('인턴'); -- 에러 발생

-- UPDATE
UPDATE VW_JOB
SET JOB_NAME = 'PARTTIME'
WHERE JOB_NAME = '사원';
...
INSERT INTO VW_EMP_SAL VALUES('400', '홍길동', 3000000, 36000000); -- ERROR
...
SELECT 
    DEPT_CODE, 
    SUM(SALARY) AS "급여합계",
    ROUND(NVL(AVG(SALARY),0)) AS "급여평균"
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
-- INSERT
INSERT INTO VW_JOINEMP VALUES(300, '조세오', '총무부'); -- ERROR
INSERT INTO VW_JOINEMP(EMP_ID, EMP_NAME) VALUES(302, '조세오');
