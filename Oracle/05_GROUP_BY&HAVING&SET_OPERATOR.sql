...
-- 성별 별(남자/여자) 사원의 수 조회
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남자', 2, '여자') AS "성별",  COUNT(*)AS "사원수"
FROM EMPLOYEE
...
SELECT JOB_CODE, ROUND(SUM(NVL(SALARY,0)))
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING ROUND(SUM(NVL(SALARY,0)))>=10000000
ORDER BY JOB_CODE;
-- 부서별 보너스를 받는 사원이 없는 부서만 조회
SELECT DEPT_CODE
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING SUM(NVL(BONUS,0))=0
ORDER BY DEPT_CODE;
...
-- 4. MINUS
-- 부서 코드가 5인 사원들 중에서 급여가 300만원 초과인 사원들을 제외해서 조회
SELECT EMP_NAME, EMP_ID, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE='D5'
MINUS
